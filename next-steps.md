...
**What's still manual / not yet built:** there's no self-serve signup UI (store creation is one API call, run by us on the merchant's behalf via `create-demo-store.js` or a direct curl/Postman call, not a webpage they sign up on themselves). No billing/subscription enforcement. Basic abuse protection (rate limiting, per-store CORS origin locking, security headers) is built and tested, but per-domain CORS locking is opt-in — a merchant has to tell us their domain at signup for it to apply, it's not enforced automatically. These are the right next things to build once there's a second or third paying pilot, not before.
